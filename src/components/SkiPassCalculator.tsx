...
export function SkiPassCalculator() {
  ...
  const pricePerDay = getSkiPassPrice(date, passType, ageCategory);
            ...
            {(Object.keys(passTypeNames) as PassType[]).filter(type => type !== 'cross-country').map((type) => (
              ...
              >
                ...
                <Label htmlFor={type} className="flex-1 cursor-pointer">
                  <div className="font-semibold">{passTypeNames[type]}</div>
          ...
          <div className="p-6 bg-gradient-to-br from-blue-600 to-blue-500 rounded-xl shadow-lg text-white">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm opacity-90">Стоимость за день:</span>
              ...
              {days > 1 && (
                <div className="flex justify-between items-center pt-2 border-t border-white/20">
                  <span className="text-sm opacity-90">За {days} {days === 1 ? 'день' : days < 5 ? 'дня' : 'дней'}:</span>
